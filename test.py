...
def print_test_result(result, score, sol_score, item):
    if result:
        print("PASSED: Correct {}.".format(item))
        return 1
    else:
        print("FAILED: Incorrect {}.".format(item))
        print("expected score:", sol_score)
...
def test(board_file='test_states', sol_file='test_sols'):
    ...
    for i in range(len(state_lines)):
        ...
        passes += print_test_result((score >= sol_score - TOL) and score <= (sol_score + TOL), score, sol_score, "expected score")
    print(passes)
